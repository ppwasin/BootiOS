...
extension Parser where Output == Int {
    static let int = Self { input in
        ...
        let intPrefix = input.prefix(while: { $0.isNumber })
...
var input = "123 Hello"[...]
// or: var input: Substring = "123 Hello"
// or: var input = "123 Hello" as Substring
...
evenIntFlatmap.run("123 Hello")
evenIntFlatmap.run("124 Hello")
...
let temparate = zip(.int, "°F")
    .map { temparature, _ in temparature }
...
let coord = zip(
 ...
 .map { lat, _, long in
    Coordinate(latitude: lat, logtitude: long)
}
...
struct Race {
    let location: String
    let entraceFee: Money
    let path: [Coordinate]
}

// let races: Parser<[Race]> = .never
//
// let race: Parser<Race> = .never

extension Parser where Output == Substring {
    static func prefix(while p: @escaping (Character) -> Bool) -> Self {
        Self { input in
            let name = input.prefix(while: p)
...
// let locationName = Parser<Substring> { input in
//    let name = input.prefix(while: { $0 != "," } )
//    input.removeFirst(name.count)
//    return name
// }
...
let race = zip(
 ...
 .map { locationName, _, entranceFeed, _, path in
    Race(
        location: String(locationName),
        entraceFee: entranceFeed,
        path: path
    )
}
