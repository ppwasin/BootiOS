...
struct ClockTime {
    ...
    func asString() -> String {
        return String(format: "%2i", hours) + ":" + String(format: "%02i", minutes) + ":" + String(format: "%02.0f", seconds)
    }
}
...
struct RotatingCard: View {
    ...
    func updateBinding(_ value: Bool) {
        ...
        if flipped != value, !flipped {
            imgIndex = imgIndex + 1 < images.count ? imgIndex + 1 : 0
        }
